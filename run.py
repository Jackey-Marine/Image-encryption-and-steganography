...
def securekey (iname):
    ...
    m, n = img.size
    print("pixels: {0}  height: {1}  width: {2}".format(m*n, m, n))
    pix = img.load()          
...
def EncAndDec(file_path):
    ...
    enc_img = encrypt(b,g,r,file_path)
    temp_time = time.time()
    enc_time = temp_time - start_time
    print(f'Encrypting time: {enc_time:.3f} s.')

    print("Decrypting...")
    dec_img = decrypt(enc_img,fx,fy,fz,file_path,Mk_e,blue,green,red)
    end_time = time.time()
    dec_time = end_time - temp_time
    print(f'Decrypting time: {dec_time:.3f} s.')

    print('-----------------------------')

    return enc_img, dec_img, enc_time, dec_time
...
if (__name__ == "__main__"):
    ...
    if image_input_mode == 0:
        ...
        enc_img, dec_img, enc_time, dec_time = EncAndDec(file_path)
        
        cv2.imwrite(("Encrypted.jpg"), enc_img)
        cv2.imwrite(("Decrypted.jpg"), dec_img)

    # A folder images
    elif image_input_mode == 1:
         
        folder_path = "Input_image"
        enc_folder = "Encrypted_image"
        dec_folder = "Decrypted_image"
        ...
        for i in range(num_files):
            ...
            file_path = os.path.join(folder_path, f"frame_{i}.jpg")

            env_img, dec_img, enc_time, dec_time = EncAndDec(file_path)

            enc_time_total = enc_time + enc_time_total
            dec_time_total = dec_time + dec_time_total

            if not os.path.exists(enc_folder):
                os.makedirs(enc_folder)
            cv2.imwrite(os.path.join(enc_folder, f"Encrypted_{i}.jpg"),env_img)

            if not os.path.exists(dec_folder):
                os.makedirs(dec_folder)
            cv2.imwrite(os.path.join(dec_folder, f"Decrypted_{i}.jpg"),dec_img)

        print(f'Total encryption time: {enc_time_total:.3f} s.')
        print(f'Average encryption time: {enc_time_total/num_files:.3f} s.')
        print(f'Total decryption time: {dec_time_total:.3f} s.')
        print(f'Average decryption time: {dec_time_total/num_files:.3f} s.')
